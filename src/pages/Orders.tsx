...
export default function Orders() {
    ...
    const fetch = async () => {
      ...
      try {
        const query = supabase
          .from("orders")
          .select(
            "id, amount, status, delivery_confirmed, delivery_method, created_at, products:product_id(name, image_url, image_emoji, type, vendor_id)",
          )
        ...
        if (ordersError) {
          if (import.meta.env.DEV) console.error("Error fetching orders:", ordersError);
        ...
        if (ratingsError) {
          if (import.meta.env.DEV) console.error("Error fetching ratings:", ratingsError);
        } else if (ratings) {
          setRatedOrders(new Set(ratings.map((r: any) => r.order_id)));
        }

        setLoading(false);
      } catch (e) {
        if (import.meta.env.DEV) console.error("Catch error in Orders fetch:", e);
  ...
  const confirmDelivery = async (orderId: string) => {
    try {
      ...
      if (import.meta.env.DEV) console.error("Catch error in confirmDelivery:", e);
  ...
  const statusBadge = (o: OrderRow) => {
    if (o.delivery_confirmed)
      return (
        <span className="flex items-center gap-1 text-green-500 text-[10px] font-mono">
    ...
    if (o.status === "processing")
      return (
        <span className="flex items-center gap-1 text-yellow-500 text-[10px] font-mono">
                ...
                <div className="flex items-center gap-3">
                  ...
                  {role === "buyer" && o.status === "processing" && !o.delivery_confirmed && (
                    <button
                      ...
                      className="flex items-center gap-1 px-2 py-1 bg-green-600 text-white text-[10px] font-mono rounded hover:bg-green-700 transition-colors"
                    >
